    ...
    <!-- card petition -->
        <div class="card">
            <div class="card-image">
              <img src="{{asset('/')}}public/upload/{{$petition->avatar}}" style="height: 400px;">
            ...
            <div class="row">
              <div class="col m12">.
                ...
                <ul class="collapsible" data-collapsible="accordion">
                  <li>
                      @foreach($petition->comments as $commnt)
@if ($commnt->comments_approved == 1)
 ...
 @if(!empty($survey))
   @if(empty($result->user_id))
       @if($survey->option2 == 'serializeData')
       ...
       <form action="{{route('survey-result')}}" method="POST">
        {{csrf_field()}}
                  ...
                  <div class="card-content white-text">
                    ...
                    <span class="card-title"><strong>{{$survey->question}}</strong></span>
                      @foreach($options as $option)
                       <input name="survey_option" type="hidden" value="{{$option}}" />
                  <input name="survey_id" type="radio" id="{{$option}}" value="{{$survey->id}}" />
         ...
         <p>Thank You For review our Survey </p>
         @endif
